      ...
      <Card>
        ...
        <CardContent className="p-4 overflow-x-auto">
          <Table className="text-xs">
            <caption className="sr-only">Teacher workload heatmap by day</caption>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                <TableHead className="text-left text-muted-foreground font-medium pb-3 pr-4 min-w-[140px] h-auto">Teacher</TableHead>
                {DAYS.map(d => (
                  <TableHead key={d} className="text-center text-muted-foreground font-medium pb-3 px-2 min-w-[52px] h-auto">{d}</TableHead>
                ))}
                <TableHead className="text-right text-muted-foreground font-medium pb-3 pl-4 h-auto">Total</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {activeTeachers.map(t => {
                ...
                return (
                  <TableRow key={t.id}>
                    ...
                    {load.map((val, i) => (
                      <TableCell key={i} className="py-2 px-2 text-center">
                        <span className={`inline-flex items-center justify-center size-7 rounded-md font-semibold text-xs ${heatColor(val)}`}>
                          {val > 0 ? val : "—"}
                        </span>
                      </TableCell>
                    ))}
                    <TableCell className="py-2 pl-4 text-right font-bold text-foreground">{total}</TableCell>
                  </TableRow>
                )
              })}
            </TableBody>
